...
export function downloadShipmentImage(order, customer, seller) {
  const itemCount = Math.max(1, (order.items || []).length);
  const canvas = document.createElement('canvas');
  canvas.width = 1240;
  canvas.height = Math.max(1754, 930 + itemCount * 86 + (order.notes ? 130 : 0));
  ...
  ctx.fillStyle = '#5C4B73'; ctx.fillRect(0, 0, canvas.width, 14);
  ctx.fillStyle = '#3F3650'; ctx.font = 'bold 46px "PingFang SC", sans-serif'; ctx.fillText('紫都 ZIDU', left, 85);
  ctx.font = '26px "PingFang SC", sans-serif'; ctx.textAlign = 'right'; ctx.fillText('仓库发货单', right, 74);
  ctx.font = '21px monospace'; ctx.fillStyle = '#756B7F'; ctx.fillText(order.orderNo || '—', right, 108); ctx.textAlign = 'left';
  ctx.strokeStyle = '#5C4B73'; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(left, 135); ctx.lineTo(right, 135); ctx.stroke();

  let y = 188;
  const section = title => { ctx.fillStyle = '#5C4B73'; ctx.font = 'bold 25px "PingFang SC", sans-serif'; ctx.fillText(title, left, y); y += 42; };
  const info = (label, value, x, width) => { ctx.fillStyle = '#8A8178'; ctx.font = '20px "PingFang SC", sans-serif'; ctx.fillText(label, x, y); ctx.fillStyle = '#342F3B'; return wrapCanvasText(ctx, value, x + 105, y, width - 105, 29); };
  section('收件信息');
  info('客户名称', customer?.name || '—', left, 500); info('收件人', customer?.contact || customer?.name || '—', 650, 500); y += 10;
  info('联系电话', customer?.phone || '—', left, 500); info('销售人员', seller?.name || '—', 650, 500); y += 10;
  y = info('收件地址', fullAddress, left, 1060) + 22;

  section('发货明细');
  const cols = [left, 300, 710, 1000, right];
  ctx.fillStyle = '#F3EFF5'; ctx.fillRect(left, y - 27, right - left, 52);
  ctx.fillStyle = '#625970'; ctx.font = 'bold 20px "PingFang SC", sans-serif';
  ['产品编号', '产品名称', '规格', '数量'].forEach((t, i) => ctx.fillText(t, cols[i] + 12, y + 6));
  y += 45;
  (order.items || []).forEach(it => {
    ctx.strokeStyle = '#E5DEE8'; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(left, y + 36); ctx.lineTo(right, y + 36); ctx.stroke();
    ctx.fillStyle = '#3A3441'; ctx.font = '20px "PingFang SC", sans-serif';
    ctx.fillText(String(it.productCode || '—'), cols[0] + 12, y + 8);
    ctx.fillText(String(it.productName || '—').slice(0, 20), cols[1] + 12, y + 8);
    ctx.fillText(String(it.spec || '—'), cols[2] + 12, y + 8);
    ctx.font = 'bold 22px "PingFang SC", sans-serif'; ctx.fillText(String(it.quantity), cols[3] + 12, y + 8);
    y += 72;
  });
  y += 24;
  section('物流信息');
  info('快递公司', shipment.carrier || '—', left, 500); info('快递单号', shipment.trackingNo || '—', 650, 500); y += 10;
  info('发货日期', shipment.shippedAt || '—', left, 500); info('操作人员', shipment.operator || '—', 650, 500); y += 35;
  if (order.notes) { section('发货备注'); ctx.fillStyle = '#514A56'; ctx.font = '20px "PingFang SC", sans-serif'; y = wrapCanvasText(ctx, order.notes, left, y, right - left, 30) + 20; }
